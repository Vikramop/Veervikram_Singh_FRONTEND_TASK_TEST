...
export default function Page() {
  ...
  const handleSignup = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const payload = { name, dob, phoneNumber: phone, passcode };
      const res = await signup(payload); // calls backend
      console.log('Signup response phoneNumber:', res.phoneNumber);

      if (res.phoneNumber) {
        localStorage.setItem('phone', res.phoneNumber);
        console.log('Phone number saved to localStorage');
      } else {
        console.warn('No phoneNumber to store in localStorage');
      }
            ...
            <>
              <form onSubmit={handleSignup} className="mt-6 space-y-4">
                ...
                <div className="flex gap-4">
                  ...
                  <button
                    type="submit"
                    // onClick={() => router.push('/signup/telegramLink')}
